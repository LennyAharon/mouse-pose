...
def diff(new: dict, old: dict) -> str:
    ...
    for name in sorted(set(nd) | set(od)):
        ...
        if a["raw_folder"] != b["raw_folder"]: changes.append(f"raw folder {a['raw_folder']} → {b['raw_folder']}")
        elif a["raw_csv_sha256"] != b["raw_csv_sha256"]: changes.append("raw label CSVs changed (same folder)")
...
def main() -> None:
    ...
    for name, d in manifest["datasets"].items():
        print(f"  {name:14s} train {d['train_frames']:5d}  test {d['test_frames']:5d}  keypoints {len(d['direct']):2d}  "
              f"views {len(d['views'])}  raw {d['raw_folder']} [{d['raw_csv_sha256']}]")
    if args.diff:
